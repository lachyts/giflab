...
def _create_test_gif(path: Path, frames: int = 10, size: tuple[int, int] = (50, 50)) -> None:
    ...
    for i in range(frames):
        img = Image.new("RGB", size, color=((i * 37) % 255, (i * 53) % 255, (i * 97) % 255))
...
def _engine_available(engine: LossyEngine) -> bool:
    ...
    if engine == LossyEngine.GIFSICLE:
        ...
        try:
            subprocess.run([binary, "--version"], capture_output=True, check=True, timeout=3)
            return True
        except Exception:
            return False
...
def _compress_with(engine: LossyEngine, src: Path, lossy_level: int, frame_ratio: float, colour_count: int | None = None):
...
def test_gifsicle_vs_animately_equivalence(test_gif_tmp, lossy_level, frame_ratio, colours):
    ...
    if not (_engine_available(LossyEngine.GIFSICLE) and _engine_available(LossyEngine.ANIMATELY)):
        pytest.skip("Both gifsicle and animately must be available for equivalence checks")
    ...
    try:
        ...
        if colours is not None:
            ...
            assert colours_gif <= colours, f"Gifsicle exceeded color limit: {colours_gif} > {colours}"
            assert colours_ani <= colours, f"Animately exceeded color limit: {colours_ani} > {colours}"
