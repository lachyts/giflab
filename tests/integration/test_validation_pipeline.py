...
class TestFullPipelineValidation:
    ...
    @pytest.mark.fast
    def test_metrics_in_full_compression_pipeline(self, validation_checker, sample_gif_files):
    ...
    def test_validation_thresholds_effectiveness(self, validation_checker):
        ...
        good_result = validation_checker.validate_compression_result(
            original_metadata, good_metrics, pipeline_id="pipeline1", gif_name="good_gif", content_type="test"
        )
        ...
        poor_result = validation_checker.validate_compression_result(
            original_metadata, poor_phase2_metrics, pipeline_id="pipeline2", gif_name="poor_gif", content_type="test"
        )
        
        # Should detect issues from Phase 2 metrics
        assert poor_result.status in [ValidationStatus.ERROR, ValidationStatus.WARNING, ValidationStatus.ARTIFACT]
        ...
        phase2_detected = any(
            any(keyword in msg.lower() for keyword in ["perceptual", "lpips", "dither", "quality"])
        ...
        quality_issues_detected = len(poor_result.issues) > 0 or len(poor_result.warnings) > 0
        
        assert phase2_detected or quality_issues_detected, f"Expected Phase 2 or quality issues to be detected. Messages: {all_messages}"
    ...
    def test_multi_metric_validation_combinations(self, validation_checker):
        ...
        result = validation_checker.validate_compression_result(
            original_metadata, mixed_metrics, pipeline_id="mixed_pipeline", gif_name="mixed_gif", content_type="test"
        )
        ...
        assert quality_related_issues, "Expected validation to detect quality issues from mixed good/poor metrics"

    @pytest.mark.fast
    def test_conditional_deep_perceptual_triggering_in_pipeline(self, validation_checker):
        ...
        high_quality_result = validation_checker.validate_compression_result(
            original_metadata, high_quality_metrics, pipeline_id="hq_pipeline", gif_name="hq_gif", content_type="test"
        )
        # Should be acceptable (PASS or WARNING are both valid for high quality metrics)
        assert high_quality_result.status in [ValidationStatus.PASS, ValidationStatus.WARNING]
        ...
        assert should_use_deep_perceptual(borderline_quality_metrics["composite_quality"])
        
        # Run validation - should include deep perceptual validation
        borderline_result = validation_checker.validate_compression_result(
            original_metadata, borderline_quality_metrics, pipeline_id="borderline_pipeline", gif_name="borderline_gif", content_type="test"
        )
...
class TestRegressionPrevention:
    ...
    def test_golden_gif_quality_scores(self, golden_reference_metrics):
        ...
        for gif_name, expected_metrics in golden_reference_metrics.items():
            ...
            if expected_status == ValidationStatus.PASS:
                ...
                assert result.status in [ValidationStatus.WARNING, ValidationStatus.ERROR, ValidationStatus.PASS]
            elif expected_status == ValidationStatus.ERROR:
                assert result.status in [ValidationStatus.ERROR, ValidationStatus.ARTIFACT, ValidationStatus.WARNING]
    ...
    def test_known_failure_detection(self):
        ...
        assert quality_failure_detected, f"Expected quality issues to be detected with catastrophic metrics. Categories: {issue_categories}"
...
class TestValidationSystemIntegration:
    ...
    def test_validation_performance_with_phase2(self):
        ...
        assert result.validation_time_ms >= 0  # 0ms is acceptable for very fast validation
    ...
    def test_phase2_error_isolation(self):
        ...
        result = validation_checker.validate_compression_result(
            original_metadata, minimal_metrics, pipeline_id="minimal_pipeline", gif_name="minimal_gif", content_type="test"
        )
        ...
        result_invalid = validation_checker.validate_compression_result(
            original_metadata, invalid_phase2_metrics, pipeline_id="invalid_pipeline", gif_name="invalid_gif", content_type="test"
        )
