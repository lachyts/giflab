...
class ParetoAnalyzer:
    ...
    def generate_comprehensive_pareto_analysis(self) -> dict[str, Any]:
        ...
        try:
            ...
            for content_type in self.results_df["content_type"].unique():
                content_data = self.results_df[
                    self.results_df["content_type"] == content_type
                ]
                analysis["content_type_frontiers"][
                    content_type
                ] = self._compute_pareto_frontier(
                    content_data,
                    quality_col="composite_quality",
                    size_col="file_size_kb",
                )
    ...
    def _compute_pareto_frontier(
        self, data: pd.DataFrame, quality_col: str, size_col: str
    ) -> dict:
        """Compute Pareto frontier for quality vs size trade-off."""

        if (
            data.empty
            or quality_col not in data.columns
            or size_col not in data.columns
        ):
        ...
        frontier_pipelines = {
            point[2] for point in global_frontier
        }  # pipeline_ids in frontier
            ...
            "frontier_points": [
                {
                    ...
                    "pipeline_id": point[2],
                    "efficiency_score": point[0] / point[1]
                    if point[1] > 0
                    else 0,  # quality per KB
    ...
    def _pipeline_dominates(self, data_a: pd.DataFrame, data_b: pd.DataFrame) -> bool:
        ...
        best_a_quality = (
            data_a["composite_quality"].max()
            if "composite_quality" in data_a.columns
            else 0
        )
        best_a_size = (
            data_a["file_size_kb"].min()
            if "file_size_kb" in data_a.columns
            else float("inf")
        )

        best_b_quality = (
            data_b["composite_quality"].max()
            if "composite_quality" in data_b.columns
            else 0
        )
        best_b_size = (
            data_b["file_size_kb"].min()
            if "file_size_kb" in data_b.columns
            else float("inf")
        )
        ...
        strictly_better = (best_a_quality > best_b_quality) or (
            best_a_size < best_b_size
        )
    ...
    def _rank_pipelines_at_quality_targets(self) -> dict:
        ...
        for target_quality in target_qualities:
            ...
            for pipeline_id in self.results_df["pipeline_id"].unique():
                pipeline_data = self.results_df[
                    self.results_df["pipeline_id"] == pipeline_id
                ]

                # Find results at or above target quality
                quality_col = "composite_quality"
                if quality_col in pipeline_data.columns:
                    quality_results = pipeline_data[
                        pipeline_data[quality_col] >= target_quality
                    ]
            ...
            ranked_pipelines = sorted(
                capable_pipelines.items(), key=lambda x: x[1]["best_size_kb"]
            )
    ...
    def _generate_trade_off_insights(self) -> dict[str, Any]:
        ...
        try:
            ...
            if (
                "composite_quality" in self.results_df.columns
                and "file_size_kb" in self.results_df.columns
            ):
                # Avoid division by zero
                valid_sizes = self.results_df["file_size_kb"] > 0
                temp_df = self.results_df[valid_sizes].copy()
                temp_df["efficiency_ratio"] = (
                    temp_df["composite_quality"] / temp_df["file_size_kb"]
                )
                ...
                for pipeline_id in temp_df["pipeline_id"].unique():
                    ...
                    if len(pipeline_data) > 2:
                        correlation = pipeline_data["composite_quality"].corr(
                            pipeline_data["file_size_kb"]
                        )
                ...
                quality_threshold = temp_df["composite_quality"].quantile(
                    0.8
                )  # Top 20% quality
                size_threshold = temp_df["file_size_kb"].quantile(
                    0.2
                )  # Bottom 20% size
                ...
                insights["sweet_spot_pipelines"] = (
                    sweet_spot_results["pipeline_id"].value_counts().to_dict()
                )
