...
class ExperimentalAnalyzer:
    ...
    def _load_results(self) -> pd.DataFrame:
        ...
        try:
            ...
            df['compression_ratio'] = df['orig_kilobytes'] / df['kilobytes'].replace(0, 1)
            df['file_size_reduction'] = (df['orig_kilobytes'] - df['kilobytes']) / df['orig_kilobytes']
            df['quality_score'] = (df['ssim'] + df['composite_quality']) / 2
    ...
    def compare_strategies(self, quality_threshold: float = 0.8) -> List[StrategyComparison]:
        ...
        """
        successful_results = self.df[self.df['success'] == True]
        ...
        for strategy in successful_results['strategy'].unique():
            ...
            success_rate = len(strategy_data) / len(self.df[self.df['strategy'] == strategy])
        ...
        comparisons.sort(
            key=lambda x: (x.avg_compression_ratio * x.quality_score * x.success_rate) - (x.avg_processing_time / 10000),
            reverse=True
        )
    ...
    def detect_anomalies(self, threshold_std: float = 2.0) -> AnomalyDetection:
        ...
        compression_outliers = successful_results[
            abs(successful_results['compression_ratio'] - compression_mean) > threshold_std * compression_std
        ]
        ...
        quality_outliers = successful_results[
            abs(successful_results['quality_score'] - quality_mean) > threshold_std * quality_std
        ]
        ...
        failure_rate = len(self.df[self.df['success'] == False]) / len(self.df)
        if failure_rate > 0.1:
            suspicious_patterns.append(f"High failure rate: {failure_rate:.1%}")
            recommendations.append("Investigate failed jobs and consider engine availability")
        
        # Check for strategy-specific issues
        for strategy in self.df['strategy'].unique():
            strategy_data = self.df[self.df['strategy'] == strategy]
            strategy_success_rate = len(strategy_data[strategy_data['success'] == True]) / len(strategy_data)
            
            if strategy_success_rate < 0.8:
                suspicious_patterns.append(f"Low success rate for {strategy}: {strategy_success_rate:.1%}")
                recommendations.append(f"Review {strategy} configuration and engine availability")
        ...
        if successful_results['quality_score'].mean() < 0.6:
            ...
            recommendations.append("Consider adjusting compression parameters or quality thresholds")
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        successful_jobs = len(self.df[self.df['success'] == True])
    ...
    def _analyze_by_parameters(self) -> Dict[str, Any]:
        ...
        if 'frame_keep_ratio' in successful_data.columns:
            ...
            for frame_ratio in successful_data['frame_keep_ratio'].unique():
                frame_data = successful_data[successful_data['frame_keep_ratio'] == frame_ratio]
        ...
        if 'color_keep_count' in successful_data.columns:
            ...
            for color_count in successful_data['color_keep_count'].unique():
                color_data = successful_data[successful_data['color_keep_count'] == color_count]
    ...
    def create_visualizations(self, output_dir: Path) -> List[Path]:
        ...
        if len(successful_data['strategy'].unique()) > 1:
            ...
            strategy_compression = successful_data.groupby('strategy')['compression_ratio'].mean()
            ...
            strategy_quality = successful_data.groupby('strategy')['quality_score'].mean()
            ...
            strategy_time = successful_data.groupby('strategy')['processing_time_ms'].mean()
        ...
        if len(successful_data) > 10:
            ...
            axes[0, 0].hist(successful_data['compression_ratio'], bins=20, alpha=0.7, edgecolor='black')
            axes[0, 0].set_title('Compression Ratio Distribution')
            axes[0, 0].set_xlabel('Compression Ratio')
            axes[0, 0].set_ylabel('Frequency')
            
            # Quality score distribution
            axes[0, 1].hist(successful_data['quality_score'], bins=20, alpha=0.7, edgecolor='black')
            axes[0, 1].set_title('Quality Score Distribution')
            axes[0, 1].set_xlabel('Quality Score')
            axes[0, 1].set_ylabel('Frequency')
            
            # Processing time distribution
            axes[1, 0].hist(successful_data['processing_time_ms'], bins=20, alpha=0.7, edgecolor='black')
            axes[1, 0].set_title('Processing Time Distribution')
            axes[1, 0].set_xlabel('Processing Time (ms)')
            axes[1, 0].set_ylabel('Frequency')
            
            # File size reduction distribution
            axes[1, 1].hist(successful_data['file_size_reduction'], bins=20, alpha=0.7, edgecolor='black')
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if not successful_data.empty:
            ...
            if avg_compression < 2.0:
                recommendations.append("Consider more aggressive compression settings to improve compression ratios")
            
            avg_quality = successful_data['quality_score'].mean()
            if avg_quality > 0.9:
                recommendations.append("High quality scores suggest room for more aggressive compression")
        
        return recommendations


def analyze_experiment_results(results_csv: Path, output_dir: Optional[Path] = None) -> Dict[str, Any]:
