...
def validate_results(csv_file: Path, output: Path | None, content_type: str) -> None:
    ...
    try:
        ...
        for idx, row in df.iterrows():
            if pd.isna(row.get("validation_status")) or row.get("validation_status") == "EXECUTION_FAILED":
                # Re-run validation for rows missing validation data
                try:
                    # Reconstruct validation inputs from CSV data
                    original_info = {
                        "frame_count": int(row.get("orig_frames", 0)) if pd.notna(row.get("orig_frames")) else 0,
                        "fps": float(row.get("orig_fps", 0)) if pd.notna(row.get("orig_fps")) else 0.0,
                        "file_size_kb": float(row.get("original_size_kb", 0)) if pd.notna(row.get("original_size_kb")) else 0.0,
                        "width": int(row.get("orig_width", 0)) if pd.notna(row.get("orig_width")) else 0,
                        "height": int(row.get("orig_height", 0)) if pd.notna(row.get("orig_height")) else 0,
                    }
                    
                    compressed_info = {
                        "frame_count": int(row.get("compressed_frame_count", 0)) if pd.notna(row.get("compressed_frame_count")) else 0,
                        "fps": float(row.get("orig_fps", 0)) if pd.notna(row.get("orig_fps")) else 0.0,  # Use original FPS as fallback
                        "file_size_kb": float(row.get("file_size_kb", 0)) if pd.notna(row.get("file_size_kb")) else 0.0,
                        "quality_metrics": {
                            "ssim": float(row.get("ssim_mean", 0)) if pd.notna(row.get("ssim_mean")) else 0.0,
                            "composite_quality": float(row.get("composite_quality", 0)) if pd.notna(row.get("composite_quality")) else 0.0,
                            "efficiency": float(row.get("efficiency", 0)) if pd.notna(row.get("efficiency")) else 0.0,
                            "temporal_consistency": float(row.get("temporal_consistency", 0)) if pd.notna(row.get("temporal_consistency")) else 0.0,
                        }
                    }
                    ...
                    metadata = GifMetadata(
                        ...
                        orig_filename=str(row.get("orig_filename", row.get("gif_name", "unknown"))),
                        orig_kilobytes=original_info["file_size_kb"],
                        orig_width=original_info["width"],
                        orig_height=original_info["height"],
                        orig_frames=original_info["frame_count"],
                        orig_fps=original_info["fps"],
                        orig_n_colors=int(row.get("orig_n_colors", 256)) if pd.notna(row.get("orig_n_colors")) else 256,
                        entropy=float(row.get("entropy", 0)) if pd.notna(row.get("entropy")) else None,
                    ...
                    validation_result = validator.validate_compression_result(
                        ...
                        compression_metrics=compressed_info["quality_metrics"],
                    ...
                    validation_updates.append({
                        ...
                        "validation_issues_count": len(validation_result.issues) + len(validation_result.warnings),
                        "validation_messages": "; ".join([issue.message for issue in (validation_result.issues + validation_result.warnings)]) if (validation_result.issues or validation_result.warnings) else None,
                    })
        ...
        click.echo(f"📊 Processed {len(validation_updates)} rows with validation updates")
...
@click.option(
    "--category",
    type=click.Choice(["fps", "quality", "artifacts", "efficiency"], case_sensitive=False),
...
def filter_results(csv_file: Path, status: str | None, category: str | None, output: Path | None) -> None:
    ...
    try:
        ...
        if status:
            ...
            click.echo(f"🔍 Filtered by status '{status_upper}': {len(filtered_df)}/{original_count} results")
        ...
        if category:
            ...
            def has_category_issue(messages):
                if pd.isna(messages):
                    return False
                messages_lower = str(messages).lower()
                return any(keyword in messages_lower for keyword in keywords)
            
            filtered_df = filtered_df[filtered_df["validation_messages"].apply(has_category_issue)]
            click.echo(f"🔍 Filtered by category '{category}': {len(filtered_df)}/{original_count} results")
        ...
        click.echo(f"📊 Results: {len(filtered_df)}/{original_count} ({len(filtered_df)/original_count*100:.1f}%)")
        
        if len(filtered_df) > 0:
            # Show validation status distribution
            validation_counts = filtered_df["validation_status"].value_counts()
            click.echo("📈 Validation status distribution:")
            for status_val, count in validation_counts.items():
                status_emoji = {"PASS": "✅", "WARNING": "⚠️", "ERROR": "❌", "ARTIFACT": "🔍"}.get(status_val, "❓")
...
def generate_report(csv_file: Path, format: str, output: Path | None) -> None:
    ...
    try:
        ...
        top_issues = sorted(issue_categories.items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        report_data = {
            ...
            "thresholds_analysis": {
                "most_failed_pipelines": df[df["validation_status"] != "PASS"]["pipeline_id"].value_counts().head(5).to_dict() if len(df[df["validation_status"] != "PASS"]) > 0 else {},
                "avg_issues_per_result": round(df["validation_issues_count"].mean(), 2) if "validation_issues_count" in df.columns else 0,
            }
        }
...
📊 SUMMARY
Total Experiments: {report_data['summary']['total_experiments']}
Validation Pass Rate: {report_data['summary']['validation_pass_rate']}%

📈 VALIDATION STATUS DISTRIBUTION
"""
            for status, count in report_data['summary']['validation_status_distribution'].items():
                status_emoji = {"PASS": "✅", "WARNING": "⚠️", "ERROR": "❌", "ARTIFACT": "🔍"}.get(status, "❓")
                percentage = (count / total_results) * 100
                output_content += f"{status_emoji} {status}: {count} ({percentage:.1f}%)\n"
            ...
            output_content += "\n⚙️ PIPELINE ANALYSIS\n"
            if report_data['thresholds_analysis']['most_failed_pipelines']:
                output_content += "Most Failed Pipelines:\n"
                for pipeline, count in list(report_data['thresholds_analysis']['most_failed_pipelines'].items())[:3]:
                    output_content += f"• {pipeline}: {count} failures\n"
            
            output_content += f"Average Issues per Result: {report_data['thresholds_analysis']['avg_issues_per_result']}\n"
...
def analyze_thresholds(csv_file: Path, suggest_params: bool) -> None:
    ...
    try:
        ...
        click.echo(f"🔍 Analyzing {len(failed_df)}/{len(df)} results with validation issues")
        ...
        for issue_type, count in violation_analysis.items():
            if count > 0:
                ...
                click.echo(f"• {issue_type.replace('_', ' ').title()}: {count} ({percentage:.1f}% of failures)")
        ...
        if suggest_params:
            ...
            if violation_analysis["quality_issues"] > len(failed_df) * 0.3:
                click.echo("• Consider reducing lossy compression levels (lower lossy values)")
                click.echo("• Try pipelines with fewer compression steps")
            
            if violation_analysis["efficiency_issues"] > len(failed_df) * 0.3:
                click.echo("• Consider more aggressive color reduction (lower color counts)")
                click.echo("• Try frame reduction pipelines to improve compression ratios")
            
            if violation_analysis["fps_issues"] > len(failed_df) * 0.3:
                click.echo("• Check frame rate preservation in frame reduction pipelines")
                click.echo("• Consider content-type specific validation thresholds")
                
            if violation_analysis["artifact_issues"] > len(failed_df) * 0.3:
                click.echo("• Avoid aggressive frame reduction for content with disposal artifacts")
                click.echo("• Consider less lossy compression for artifact-prone content")
...
def get_validation_summary(csv_file: Path) -> dict:
    ...
    return {
        ...
        "avg_issues_per_result": df["validation_issues_count"].mean() if "validation_issues_count" in df.columns else 0,
        "most_failed_pipelines": df[df["validation_status"] != "PASS"]["pipeline_id"].value_counts().head(3).to_dict() if len(df[df["validation_status"] != "PASS"]) > 0 else {}
    }


def filter_validation_results(csv_file: Path, status: str | None = None, category: str | None = None) -> pd.DataFrame:
