...
def handle_error(
    ...
    if error_context:
        context_str = ", ".join(f"{k}={v}" for k, v in error_context.items() if k not in ['has_traceback'])
        if context_str:
