...
class ParallelFrameGenerator:
    ...
    def generate_frames_parallel(self, 
        ...
        self.logger.info(f"Starting parallel frame generation: {self._total_tasks} tasks "
                ...
                for future in as_completed(future_to_task):
                    try:
                        ...
                        if result.success:
                            ...
                            self.logger.warning(f"Frame generation failed: {result.error_message}")
                        
                        # Progress callback
                        if progress_callback:
                            progress_callback(self._completed_tasks + self._failed_tasks, 
                                            self._total_tasks)
                        
                    except Exception as e:
                        self._failed_tasks += 1
                        task = future_to_task[future]
                        self.logger.error(f"Frame generation exception for task {task.task_id}: {e}")
        ...
        self.logger.info(f"Parallel frame generation completed: {self._completed_tasks} successful, "
                        f"{self._failed_tasks} failed in {elapsed_time:.2f}s "
                        f"({self._total_tasks/elapsed_time:.1f} frames/sec)")
        
        return results
    
    def generate_gif_frames_parallel(self, 
        ...
        for result in results:
            if result.success and result.image_data:
                ...
                self.logger.warning(f"Using fallback for failed frame {result.frame_index}")
                # Create a fallback image
                fallback = Image.new('RGB', spec.size, (255, 0, 0))  # Red error frame
                images.append(fallback)
...
class ParallelPipelineExecutor:
    ...
    def __init__(self, 
        ...
        """
        self.max_workers = max_workers or max(1, mp.cpu_count() - 1)  # Leave one CPU for coordination
