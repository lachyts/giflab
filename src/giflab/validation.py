...
def validate_raw_dir(raw_dir: Union[str, Path], require_gifs: bool = True) -> Path:
...
def validate_path_security(path: Union[str, Path]) -> Path:
...
def validate_output_path(path: Union[str, Path], create_parent: bool = True) -> Path:
...
def validate_file_extension(path: Union[str, Path], expected_extensions: List[str]) -> Path:
