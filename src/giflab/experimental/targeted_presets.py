...
class SlotConfiguration:
    ...
    implementation: Optional[str] = None        # For locked slots: "ffmpeg-color", "animately-advanced"
    scope: Optional[List[str]] = None          # For variable slots: ["*"] or ["gifski", "animately"]  
    parameters: Dict[str, Any] = field(default_factory=dict)  # Algorithm-specific parameters
    ...
    def __post_init__(self):
        ...
        if self.type == "locked":
            ...
            if self.scope is not None:
                raise ValueError("Locked slots cannot have a scope (use implementation instead)")
        elif self.type == "variable":
            if not self.scope:
                raise ValueError("Variable slots must specify a scope")
            if self.implementation is not None:
                raise ValueError("Variable slots cannot have an implementation (use scope instead)")
        else:
            raise ValueError(f"Invalid slot type: {self.type}. Must be 'variable' or 'locked'")
...
class ExperimentPreset:
    ...
    custom_sampling: Optional[str] = None          # "quick", "representative", etc.
    max_combinations: Optional[int] = None         # Hard limit on generated pipelines
    
    # Metadata
    tags: List[str] = field(default_factory=list)  # ["research", "comparison", "optimization"]
    author: Optional[str] = None                    # Preset creator
    ...
    def __post_init__(self):
        ...
        variable_count = sum(1 for slot in [self.frame_slot, self.color_slot, self.lossy_slot] 
                           if slot.type == "variable")
        if variable_count == 0:
            raise ValueError("At least one slot must be variable (otherwise use generate_all_pipelines)")
        ...
        if self.custom_sampling:
            ...
            if self.custom_sampling not in SAMPLING_STRATEGIES:
                ...
                raise ValueError(f"Invalid sampling strategy: {self.custom_sampling}. "
    ...
    def estimate_pipeline_count(self) -> int:
        ...
        count = 1
        for slot_name, slot in [("frame", self.frame_slot), ("color", self.color_slot), ("lossy", self.lossy_slot)]:
            if slot.type == "variable":
...
class PresetRegistry:
    ...
    def find_similar_presets(self, preset: ExperimentPreset, threshold: float = 0.8) -> List[str]:
    ...
    def _calculate_preset_similarity(self, preset1: ExperimentPreset, preset2: ExperimentPreset) -> float:
        ...
        for slot1, slot2 in zip(slots1, slots2):
            if slot1.type == slot2.type:
                if slot1.type == "locked" and slot1.implementation == slot2.implementation:
                    matches += 1
