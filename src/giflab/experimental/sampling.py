...
class PipelineSampler:
    ...
    def select_pipelines_intelligently(
        ...
        sampling_config = SAMPLING_STRATEGIES.get(
            strategy, SAMPLING_STRATEGIES["representative"]
        )
        ...
        if strategy == "factorial":
            ...
            return self._progressive_elimination_sampling(
                all_pipelines, sampling_config
            )
    ...
    def _representative_sampling(
        self, all_pipelines: list, config: SamplingStrategy
    ) -> list:
        ...
        for pipeline in all_pipelines:
            if hasattr(pipeline, "steps") and hasattr(pipeline.steps, "__iter__"):
                try:
                    ...
                    self.logger.warning(
                        f"⚠️ Invalid pipeline object detected: {type(pipeline)}. Skipping."
                    )
            else:
                self.logger.warning(
                    f"⚠️ Pipeline object missing 'steps' attribute: {type(pipeline)}. Skipping."
                )
        ...
        self.logger.info(
            f"✅ Selected {actual_count} pipelines from {len(all_pipelines)} total ({percentage:.1f}%)"
        )
    ...
    def _factorial_design_sampling(
        self, all_pipelines: list, config: SamplingStrategy
    ) -> list:
        ...
        combinations_needed = min(
            target_count, len(tool_families) * len(dithering_categories) * 3 * 2
        )  # 3 color levels, 2 lossy levels

        self.logger.info(
            f"🔬 Factorial design: {len(tool_families)} tools × {len(dithering_categories)} dithering methods"
        )
    ...
    def _progressive_elimination_sampling(
        self, all_pipelines: list, config: SamplingStrategy
    ) -> list:
        ...
        stage1_config = SamplingStrategy(
            "stage1", "Initial screening", 0.05, min_samples_per_tool=2
        )
        stage1_pipelines = self._representative_sampling(all_pipelines, stage1_config)

        self.logger.info(
            f"🔍 Stage 1: Screening {len(stage1_pipelines)} pipelines for initial assessment"
        )
        ...
        self.logger.info(
            f"📊 Progressive sampling selected {len(stage1_pipelines)} pipelines"
        )
        return stage1_pipelines

    def _targeted_expansion_sampling(
        self, all_pipelines: list, config: SamplingStrategy
    ) -> list:
        ...
        self.logger.info(
            f"📊 Targeted expansion selected {len(selected_pipelines)} pipelines"
        )
        self.logger.info(
            "🎯 Will test on strategically selected GIF subset (17 vs 25 GIFs)"
        )
    ...
    def get_targeted_synthetic_gifs(self) -> list[Path]:
        ...
        self.logger.info(
            f"   📋 Categories: {len(original_names)} original + {len(size_variation_names)} size + {len(frame_variation_names)} frames + {len(new_content_names)} content"
        )
