...
class TargetedPipelineGenerator:
    ...
    def generate_targeted_pipelines(self, preset: ExperimentPreset) -> List[Pipeline]:
        ...
        # Log generation plan
        self.logger.info(f"📊 Pipeline generation plan:")
        self.logger.info(f"   Frame tools: {len(frame_tools)} ({preset.frame_slot.type})")
        self.logger.info(f"   Color tools: {len(color_tools)} ({preset.color_slot.type})")  
        self.logger.info(f"   Lossy tools: {len(lossy_tools)} ({preset.lossy_slot.type})")
        ...
        if total_combinations == 0:
            raise RuntimeError(f"No valid tool combinations found for preset: {preset.name}")
        
        # Generate pipeline combinations
        pipelines: List[Pipeline] = []
        
        for frame_tool, color_tool, lossy_tool in itertools.product(frame_tools, color_tools, lossy_tools):
            pipeline_steps = [
        ...
        if preset.max_combinations and len(pipelines) > preset.max_combinations:
            self.logger.info(f"⚠️  Limiting pipelines to {preset.max_combinations} (from {len(pipelines)})")
            pipelines = pipelines[:preset.max_combinations]
        
        self.logger.info(f"✅ Generated {len(pipelines)} targeted pipelines")
        return pipelines
    
    def _resolve_slot_tools(self, variable: str, slot_config: SlotConfiguration) -> List[type[ExternalTool]]:
        ...
        if slot_config.type == "locked":
            ...
            return self._resolve_locked_implementation(variable, slot_config.implementation)
    ...
    def _resolve_locked_implementation(self, variable: str, implementation: str) -> List[type[ExternalTool]]:
        ...
        raise ValueError(f"Implementation '{implementation}' not found for {variable}. "
                        f"Available: {available_names}")
    
    def _expand_variable_scope(self, variable: str, scope: List[str]) -> List[type[ExternalTool]]:
        ...
        if "*" in scope:
            ...
            self.logger.debug(f"🔍 Variable {variable} expanded to all {len(available_tools)} tools")
        ...
        for tool_name in scope:
            for tool_cls in available_tools:
                ...
                # Tool name not found
                available_names = [tool.NAME for tool in available_tools]
                self.logger.warning(f"Tool '{tool_name}' not found for {variable}. "
                                  f"Available: {available_names}")
        
        if not matched_tools:
            available_names = [tool.NAME for tool in available_tools]
            raise RuntimeError(f"No tools found in scope {scope} for {variable}. "
                             f"Available: {available_names}")
        
        self.logger.debug(f"🔍 Variable {variable} matched {len(matched_tools)} tools from scope")
    ...
    def validate_preset_feasibility(self, preset: ExperimentPreset) -> Dict[str, Any]:
        ...
        try:
            ...
            for slot_name, variable in [("frame", "frame_reduction"), ("color", "color_reduction"), ("lossy", "lossy_compression")]:
            ...
            if results["valid"]:
                ...
                if estimated_count < baseline_count:
                    results["efficiency_gain"] = 1.0 - (estimated_count / baseline_count)
