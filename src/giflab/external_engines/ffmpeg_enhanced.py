...
def test_bayer_scale_performance(input_path: Path, colors: int = 16) -> dict[str, dict[str, Any]]:
    ...
    results = {}
    bayer_methods = [method for method in FFMPEG_DITHERING_METHODS if method.startswith("bayer")]
...
def validate_sierra2_vs_floyd_steinberg(test_gifs: list[Path]) -> dict[str, dict[str, Any]]:
        ...
        for method in ["floyd_steinberg", "sierra2"]:
            try:
                with tempfile.TemporaryDirectory() as tmpdir:
                    ...
                    result = color_reduce_with_dithering(
                        gif_path, output_path,
                        colors=16, dithering_method=method
                    )
