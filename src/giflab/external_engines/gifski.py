...
def lossy_compress(
    ...
    *,
    ...
    if png_sequence_dir is not None:
        ...
        if not frame_files:
            raise RuntimeError(
                f"gifski failed: no PNG frames found in {png_sequence_dir}"
            )
...
def _normalize_frame_dimensions(frame_files: list) -> list:
    ...
    for frame_file in frame_files:
        try:
            ...
            logger.warning(
                f"Could not read frame {frame_file} for dimension analysis: {e}"
            )
    ...
    logger.info(
        f"gifski: Normalizing frame dimensions to {target_dimension} "
    ...
    if temp_dir:
        logger.debug(
            f"gifski: Created {len([f for f in normalized_files if temp_dir in str(f)])} normalized frames in {temp_dir}"
        )
...
def _validate_and_prepare_frames(
    ...
    if len(unique_dimensions) > 1:
        ...
        required_consistent_frames = len(valid_frames) * (
            1.0 - max_dimension_inconsistency_ratio
        )
        ...
        logger.info(
            f"gifski: Proceeding with minor dimension inconsistencies (mostly {most_common_dim})"
        )

    logger.debug(
        f"gifski: Validated {len(valid_frames)} frames, {invalid_count} invalid frames filtered"
    )
    return valid_frames
