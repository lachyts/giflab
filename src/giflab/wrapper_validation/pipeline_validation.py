...
class PipelineStageValidator:
    ...
    def validate_frame_reduction_stage(
        ...
        if frame_ratio < 1.0:
            ...
            validations.append(ValidationResult(
                ...
                error_message=None if is_valid else f"Frame reduction in pipeline: expected ~{expected_frames} frames, got {output_frames}",
                details={
        ...
        if target_colors:
            ...
            if reduction_expected and not reduction_occurred:
                validations.append(ValidationResult(
                    ...
                    expected={"colors_reduced": True, "target_colors": target_colors},
                    actual={"colors_reduced": False, "output_colors": output_colors},
    ...
    def validate_inter_stage_consistency(
        ...
        input_width, input_height = input_metadata.orig_width, input_metadata.orig_height
        output_width, output_height = output_metadata.orig_width, output_metadata.orig_height
        
        dimensions_changed = (input_width != output_width) or (input_height != output_height)
        
        # Only certain operations should change dimensions
        dimension_change_allowed = pipeline_step.variable in ["frame_reduction"]  # Could add more if needed
        ...
        if input_fps > 0 and output_fps > 0:
            ...
            fps_consistent = fps_change_percent <= self.pipeline_thresholds["stage_fps_tolerance"]
            ...
            if not fps_consistent and not fps_change_expected:
                validations.append(ValidationResult(
                    ...
                    actual={"fps": output_fps, "fps_change_percent": fps_change_percent},
                    error_message=f"Unexpected FPS change in {pipeline_step.variable}: {input_fps:.2f} → {output_fps:.2f} ({fps_change_percent:.1%})",
                    details={
                        "stage_variable": pipeline_step.variable,
                        "fps_tolerance": self.pipeline_thresholds["stage_fps_tolerance"]
                    }
                ))
            else:
                validations.append(ValidationResult(
    ...
    def validate_pipeline_integrity(
        ...
        try:
            ...
            frame_ratio = pipeline_params.get("frame_ratio", 1.0)
            if frame_ratio < 1.0:
                expected_final_frames = max(1, int(input_metadata.orig_frames * frame_ratio))
            
            actual_final_frames = final_metadata.orig_frames
            frame_diff = abs(actual_final_frames - expected_final_frames)
            frames_valid = frame_diff <= self.pipeline_thresholds["frame_count_tolerance"]
            ...
            validations.append(ValidationResult(
                ...
                error_message=None if frames_valid else f"Pipeline final frame count mismatch: expected {expected_final_frames}, got {actual_final_frames}",
                details={
            ...
            if target_colors:
                ...
                color_target_met = final_colors <= target_colors + self.wrapper_validator.config.COLOR_COUNT_TOLERANCE
                ...
                validations.append(ValidationResult(
                    ...
                    error_message=None if color_target_met else f"Pipeline color target not met: expected ≤{target_colors}, got {final_colors}",
                    details={
    ...
    def validate_png_sequence_integrity(
        ...
        try:
            ...
            frame_count_valid = frame_diff <= self.pipeline_thresholds["png_sequence_frame_tolerance"]
            ...
            if not frame_count_valid:
                return ValidationResult(
                    ...
                    details={
                        "frame_difference": frame_diff,
                        "tolerance": self.pipeline_thresholds["png_sequence_frame_tolerance"],
            ...
            for i in range(sample_frames):
                ...
                try:
                    with Image.open(png_file) as img:
                        if img.format != "PNG":
                            return ValidationResult(
                                ...
                                details={"invalid_file": str(png_file), "detected_format": img.format}
                            )
                    ...
                    return ValidationResult(
                        ...
                        details={"corrupted_file": str(png_file), "error": str(img_error)}
                    )
