...
class WrapperOutputValidator:
    ...
    def validate_frame_reduction(
        ...
        try:
            ...
            expected_frames = max(
                self.config.MIN_FRAMES_REQUIRED, int(original_frames * expected_ratio)
            )
            actual_ratio = (
                output_frames / original_frames if original_frames > 0 else 1.0
            )

            # Validation logic with tolerance
            ratio_diff = abs(actual_ratio - expected_ratio)
            is_valid = (
                ratio_diff <= self.config.FRAME_RATIO_TOLERANCE
                and output_frames >= self.config.MIN_FRAMES_REQUIRED
            )
            ...
            return ValidationResult(
                ...
                error_message=None
                if is_valid
                else f"Frame ratio {actual_ratio:.3f} differs from expected {expected_ratio:.3f} by {ratio_diff:.3f} (tolerance: {self.config.FRAME_RATIO_TOLERANCE})",
                details={
    ...
    def validate_color_reduction(
        ...
        try:
            ...
            # Validation checks
            color_count_valid = actual_colors <= (
                expected_colors + self.config.COLOR_COUNT_TOLERANCE
            )

            # Check that significant color reduction occurred
            reduction_percent = (
                (original_colors - actual_colors) / original_colors
                if original_colors > 0
                else 0
            )
            reduction_occurred = (
                reduction_percent >= self.config.MIN_COLOR_REDUCTION_PERCENT
            )

            is_valid = color_count_valid and (
                reduction_occurred or expected_colors >= original_colors
            )

            error_parts = []
            if not color_count_valid:
                error_parts.append(
                    f"Color count {actual_colors} exceeds expected {expected_colors} + tolerance {self.config.COLOR_COUNT_TOLERANCE}"
                )
            if not reduction_occurred and expected_colors < original_colors:
                error_parts.append(
                    f"Insufficient color reduction: {reduction_percent:.1%} < {self.config.MIN_COLOR_REDUCTION_PERCENT:.1%}"
                )
    ...
    def validate_timing_preservation(
        self, input_path: Path, output_path: Path, wrapper_metadata: dict[str, Any]
    ) -> ValidationResult:
        ...
        try:
            ...
            if not fps_in_bounds:
                error_parts.append(
                    f"FPS {output_fps:.2f} outside valid range [{self.config.MIN_FPS}, {self.config.MAX_FPS}]"
                )
            if not fps_preserved:
                error_parts.append(
                    f"FPS changed from {input_fps:.2f} to {output_fps:.2f} (diff: {fps_diff:.1%}, tolerance: {self.config.FPS_TOLERANCE:.1%})"
                )
    ...
    def validate_file_integrity(
        self, output_path: Path, wrapper_metadata: dict[str, Any]
    ) -> ValidationResult:
    ...
    def validate_wrapper_output(
        ...
        timing_result = self.validate_timing_preservation(
            input_path, output_path, wrapper_metadata
        )
