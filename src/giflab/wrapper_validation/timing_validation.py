...
class TimingGridValidator:
    ...
    def __init__(self, grid_ms: int = 10):
        """Initialize timing grid validator.
        
        Args:
            grid_ms: Grid size in milliseconds for timing alignment (default: 10ms)
        """
        self.grid_ms = grid_ms
        self.timing_thresholds = {
            "max_drift_ms": 100,           # Maximum acceptable timing drift
            "duration_diff_threshold": 50,  # Maximum total duration difference
            "alignment_accuracy_min": 0.9,  # Minimum alignment accuracy
        }
            ...
            with Image.open(gif_path) as img:
                ...
                for i in range(total_frames):
                    ...
                    if duration < 1:
                        ...
                        duration = min(duration, 10000)  # Cap at 10 seconds
    ...
    def calculate_timing_drift(self, original_durations: list[int], 
                              compressed_durations: list[int]) -> dict[str, Any]:
        ...
        for i, (orig_dur, comp_dur) in enumerate(zip(original, compressed)):
    ...
    def calculate_alignment_accuracy(self, original_durations: list[int],
                                   compressed_durations: list[int]) -> float:
    ...
    def validate_timing_integrity(self, original_gif: Path, 
                                 compressed_gif: Path) -> ValidationResult:
        ...
        try:
            ...
            drift_metrics = self.calculate_timing_drift(original_durations, compressed_durations)
            ...
            is_valid = (
                max_drift <= self.timing_thresholds["max_drift_ms"] and
                duration_diff <= self.timing_thresholds["duration_diff_threshold"] and
                alignment_accuracy >= self.timing_thresholds["alignment_accuracy_min"]
            )
            ...
            if not is_valid:
                ...
                if max_drift > self.timing_thresholds["max_drift_ms"]:
                    error_parts.append(f"timing drift {max_drift}ms exceeds threshold {self.timing_thresholds['max_drift_ms']}ms")
                if duration_diff > self.timing_thresholds["duration_diff_threshold"]:
                    error_parts.append(f"duration difference {duration_diff}ms exceeds threshold {self.timing_thresholds['duration_diff_threshold']}ms")
                if alignment_accuracy < self.timing_thresholds["alignment_accuracy_min"]:
                    error_parts.append(f"alignment accuracy {alignment_accuracy:.2%} below minimum {self.timing_thresholds['alignment_accuracy_min']:.2%}")
            ...
            return ValidationResult(
                ...
                expected={
                    ...
                    "duration_diff_threshold": self.timing_thresholds["duration_diff_threshold"],
                    "alignment_accuracy_min": self.timing_thresholds["alignment_accuracy_min"],
                    "grid_ms": self.grid_ms
                },
                actual={
                    "max_timing_drift_ms": max_drift,
...
def validate_frame_timing_for_operation(original_gif: Path, compressed_gif: Path,
                                       operation_type: str = "compression",
                                       grid_ms: int = 10) -> ValidationResult:
    ...
    Args:
        ...
        grid_ms: Timing grid size in milliseconds
        
    Returns:
        ValidationResult for timing validation
    """
    validator = TimingGridValidator(grid_ms=grid_ms)
...
def extract_timing_metrics_for_csv(validation_result: ValidationResult) -> dict[str, Any]:
    ...
    """
    if not validation_result.details or "timing_metrics" not in validation_result.details:
