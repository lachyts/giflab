...
def add_validation_to_result(
    ...
    try:
        ...
        if (
            validation_config.FAIL_ON_VALIDATION_ERROR
            and not enhanced_result["validation_passed"]
        ):
            raise RuntimeError(
                f"Wrapper validation failed: {[v.error_message for v in validations if not v.is_valid]}"
            )
...
def validate_pipeline_execution_result(
    ...
    try:
        ...
        enhanced_result.update(
            {
                "pipeline_validations": [
                    _validation_result_to_dict(v) for v in validations
                ],
...
def create_validation_report(
    validations: list[ValidationResult], context: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    return {
        ...
        "failed_validation_messages": [
            v.error_message for v in validations if not v.is_valid and v.error_message
        ],
        "context": context or {},
    }
...
def _create_pipeline_validation_summary(
    validations: list[ValidationResult], pipeline: Any
) -> dict[str, Any]:
...
def _extract_stage_outputs_from_result(
    pipeline_result: dict[str, Any]
) -> dict[str, Path]:
...
def _extract_stage_metadata_from_result(
    pipeline_result: dict[str, Any], pipeline: Any
    ...
    for step in pipeline.steps:
        ...
        if (
            "stage_metadata" in pipeline_result
            and stage_name in pipeline_result["stage_metadata"]
        ):
