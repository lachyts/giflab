...
def find_results_file(custom_path: str = None, config: MonitorConfig = None) -> Path:
    ...
    return (
        ...
        else Path("elimination_results/latest/streaming_results.csv")
    )
...
def calculate_processing_rate(
    current_count: int, config: MonitorConfig = None
) -> tuple:
...
def monitor_elimination(
    ...
    while True:
        ...
        try:
            ...
            fallback_locations = (
                config.get_search_locations()
                if config
                else ["elimination_results/latest/streaming_results.csv"]
            )
        ...
        if not results_file.exists():
            ...
            try:
                ...
                if len(lines) <= 1:
                    ...
                    current_rate, trend, is_processing = calculate_processing_rate(
                        total, config
                    )
                    ...
                    if len(lines) > 1:
                        ...
                        is_likely_complete = (
                            len(recent_gifs) <= 2 and current_rate < 0.1
                        )  # Very few unique GIFs + low rate

                    status_emoji = "🏁" if is_likely_complete else "🔄"
                    status_text = "COMPLETED" if is_likely_complete else "RUNNING"

                    print(
                        f"🔬 Pipeline Elimination Progress - {status_emoji} {status_text}"
                    )
                    print("━" * 50)
                    ...
                    if current_rate > 0:
                        print(
                            f"⚡ Processing Rate: {current_rate:.1f} results/min {trend}"
                        )
                    else:
                    ...
                    if not is_likely_complete:
                        ...
                        if not is_processing:
                            print(
                                "   💡 System is likely batching results (updates every ~15-25 tests)"
                            )
                    else:
                    ...
                    if not is_likely_complete and current_rate > 0:
                        ...
                        eta_str = (
                            f", ETA: {estimated_time_remaining:.0f}min"
                            if estimated_time_remaining < 1000
                            else ""
                        )
                        print(
                            f"📋 Estimated Progress: {progress_pct:.1f}% "
...
Examples:
  ...
  python simple_monitor.py --create-config          # Create sample config file
    ...
    parser.add_argument(
        "--config", "-c", type=str, default=None, help="Path to JSON configuration file"
    )
    ...
    final_refresh = (
        ...
        else (config.refresh_interval if config else 30)
    )
    final_failures = (
        args.failures
        if args.failures is not None
        else (config.failures_to_show if config else 3)
    )
