...
def load_dataset():
    ...
    return df[df['success'] == True].copy()  # Focus on successful results
...
def dataset_overview(df):
    ...
    print(f"   • Date range: Single experiment run (run_20250807_123641)")
    ...
    print(f"\n💾 File Size Analysis:")
    print(f"   • Original sizes: {df['original_size_kb'].min():.1f}KB to {df['original_size_kb'].max():.1f}KB")
    print(f"   • Compressed sizes: {df['file_size_kb'].min():.1f}KB to {df['file_size_kb'].max():.1f}KB")
...
def pipeline_architecture_analysis(df):
    ...
    print(f"🔧 Frame Reduction Components Tested:")
...
def quality_metrics_breakdown(df):
    ...
    for metric_col, metric_name in quality_metrics:
        if metric_col in df.columns:
            ...
            print(f"   • {metric_name.ljust(12)}: {stats['mean']:.3f} ± {stats['std']:.3f} (range: {stats['min']:.3f}-{stats['max']:.3f})")
...
def efficiency_analysis(df):
    ...
    df['efficiency_tier'] = pd.cut(df['efficiency'], 
                                 bins=[0, 1, 2.5, 5, 10, float('inf')],
                                 labels=['Poor (0-1)', 'Fair (1-2.5)', 'Good (2.5-5)', 'Excellent (5-10)', 'Outstanding (10+)'],
                                 include_lowest=True)
...
def content_type_deep_dive(df):
    ...
    content_analysis.columns = [f"{col[1]}_{col[0]}" if col[1] else col[0] for col in content_analysis.columns]
    content_analysis = content_analysis.sort_values('mean_efficiency', ascending=False)
    
    print(f"📈 Performance Rankings (by average efficiency):")
    for idx, (content_type, row) in enumerate(content_analysis.iterrows(), 1):
        ...
        print(f"    Efficiency: {row['mean_efficiency']:5.2f} ± {row['std_efficiency']:4.2f} (max: {row['max_efficiency']:5.2f})")
        print(f"    Quality:    {row['mean_enhanced_composite_quality']:5.3f} ± {row['std_enhanced_composite_quality']:5.3f}")
        print(f"    Compression:{row['mean_compression_ratio']:5.1f}x ± {row['std_compression_ratio']:4.1f} (max: {row['max_compression_ratio']:5.1f}x)")
        print(f"    File size:  {row['mean_file_size_kb']:5.1f}KB ± {row['std_file_size_kb']:5.1f}")
...
def champion_pipelines(df):
    ...
    categories = {
        ...
        "Best Balance": df.loc[(df['enhanced_composite_quality'] * df['compression_ratio']).idxmax()]
    }
    ...
    for category, champion in categories.items():
        ...
        print(f"   Size: {champion['file_size_kb']:.1f}KB (was {champion['original_size_kb']:.1f}KB)")
...
def performance_insights(df):
    ...
    tool_performance = df.groupby('frame_tool').agg({
        'efficiency': 'mean',
        'enhanced_composite_quality': 'mean',
        'compression_ratio': 'mean'
    }).round(3).sort_values('efficiency', ascending=False)
    
    print(f"🔧 Frame Reduction Tool Rankings:")
    for idx, (tool, performance) in enumerate(tool_performance.iterrows(), 1):
        print(f"   {idx}. {tool.ljust(12)}: Efficiency {performance['efficiency']:5.2f} | Quality {performance['enhanced_composite_quality']:5.3f} | Compression {performance['compression_ratio']:4.1f}x")
    ...
    print(f"   Motion/Animation Content:")
    for content in motion_like:
        if content in content_champions:
            champ = content_champions[content]
            print(f"     • {content.ljust(12)}: {champ['pipeline']} (eff: {champ['efficiency']:.1f})")
    
    print(f"   Complex Visual Content:")
    for content in complex_like:
        if content in content_champions:
            champ = content_champions[content]
            print(f"     • {content.ljust(12)}: {champ['pipeline']} (eff: {champ['efficiency']:.1f})")
    
    print(f"   Simple/Geometric Content:")
...
def statistical_summary(df):
    ...
    print(f"   • Success rate: {(df['success'] == True).sum()}/{len(df)} (100.0%)")
    print(f"   • Average processing time: {df['total_processing_time_ms'].mean():.0f}ms")
    print(f"   • Total processing time: {df['total_processing_time_ms'].sum()/1000/60:.1f} minutes")
    
    print(f"\n📈 Performance Distributions:")
    print(f"   • Quality correlation (legacy vs enhanced): {df['composite_quality'].corr(df['enhanced_composite_quality']):.3f}")
    print(f"   • Files achieving >10x compression: {(df['compression_ratio'] > 10).sum()} ({(df['compression_ratio'] > 10).mean()*100:.1f}%)")
    print(f"   • Files achieving >0.8 quality: {(df['enhanced_composite_quality'] > 0.8).sum()} ({(df['enhanced_composite_quality'] > 0.8).mean()*100:.1f}%)")
    print(f"   • High-efficiency results (>5): {(df['efficiency'] > 5).sum()} ({(df['efficiency'] > 5).mean()*100:.1f}%)")
    
    print(f"\n🎯 Key Metrics:")
    print(f"   • Median file size reduction: {(1 - df['file_size_kb'] / df['original_size_kb']).median()*100:.1f}%")
    print(f"   • Average quality retention: {df['enhanced_composite_quality'].mean()*100:.1f}%")
