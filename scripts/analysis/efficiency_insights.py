...
def show_efficiency_in_practice(df):
    ...
    for idx, row in samples.iterrows():
        ...
        print(f"   Pipeline: {row['frame_tool']} + {row['color_tool']} + {row['lossy_tool']}")
        print(f"   Compression: {row['compression_ratio']:.1f}x")
        print(f"   Quality: {row['enhanced_composite_quality']:.3f}")
        print(f"   Efficiency: {row['compression_ratio']:.1f} × {row['enhanced_composite_quality']:.3f} = {efficiency_calc:.2f}")
        print(f"   Result: {row['file_size_kb']:.1f}KB (was {row['original_size_kb']:.1f}KB)")
...
def compare_old_vs_new_quality_assessment(df):
    ...
    big_improvements = df[df['quality_improvement'] > 0.3].nlargest(3, 'quality_improvement')
    big_decreases = df[df['quality_improvement'] < -0.1].nsmallest(3, 'quality_improvement')
    
    print("Cases where ENHANCED metrics found BETTER quality:")
    for idx, row in big_improvements.iterrows():
        print(f"\n✅ {row['content_type'].title()} - {row['frame_tool']} pipeline")
        print(f"   Legacy assessment: {row['composite_quality']:.3f} (based on 4 metrics)")
        print(f"   Enhanced assessment: {row['enhanced_composite_quality']:.3f} (based on 11 metrics)")
        print(f"   Improvement: +{row['quality_improvement']:.3f}")
        print(f"   Why: Enhanced system detected quality in dimensions legacy system missed")
        print(f"   Impact: Efficiency score rose to {row['efficiency']:.2f}")
    
    print(f"\n\nCases where ENHANCED metrics found LOWER quality:")
    for idx, row in big_decreases.iterrows():
        ...
        print(f"   Why: Enhanced system detected quality issues in additional dimensions")
...
def find_best_pipelines_by_use_case(df):
    ...
    use_cases = {
        ...
        "Balanced Performance": df.loc[df['balanced_score'].nlargest(3).index] if 'balanced_score' in df.columns else df.nlargest(3, 'efficiency')
    }
    
    for use_case, results in use_cases.items():
        print(f"\n🏆 {use_case}:")
        for idx, (_, row) in enumerate(results.iterrows(), 1):
            print(f"   {idx}. {row['frame_tool']}+{row['color_tool']}+{row['lossy_tool']}")
...
def content_type_recommendations(df):
    ...
    for content_type in df['content_type'].unique():
        ...
        best_quality = content_data.loc[content_data['enhanced_composite_quality'].idxmax()]
    ...
    sorted_content = sorted(content_insights.items(), 
    ...
    for content_type, data in sorted_content[:8]:  # Top 8 to keep output reasonable
        ...
        print(f"   🥇 Efficiency winner: {eff_champ['frame_tool']}+{eff_champ['lossy_tool']} (score: {eff_champ['efficiency']:.2f})")
        if eff_champ['pipeline_id'] != qual_champ['pipeline_id']:
            print(f"   🏆 Quality winner: {qual_champ['frame_tool']}+{qual_champ['lossy_tool']} (quality: {qual_champ['enhanced_composite_quality']:.3f})")
        else:
            print(f"   ✨ Same pipeline wins both efficiency and quality!")
...
def efficiency_distribution_insights(df):
    ...
    tier_analysis = df.groupby('efficiency_tier').agg({
    ...
    if len(outstanding) > 0:
        print(f"   Outstanding (10+): {len(outstanding)} results - Perfect for web optimization")
        top_outstanding = outstanding.iloc[0]
        print(f"   Example: Achieved {top_outstanding['compression_ratio']:.1f}x compression with {top_outstanding['enhanced_composite_quality']:.3f} quality")
    ...
    if len(good) > 0:
        print(f"   Good (2.5-5): {len(good)} results - Solid performance for most applications")
        
    fair = df[(df['efficiency'] >= 1) & (df['efficiency'] < 2.5)]
    if len(fair) > 0:
        print(f"   Fair (1-2.5): {len(fair)} results - May sacrifice too much quality or compression")
...
def main():
    ...
    print(f"\n\n🎉 SUMMARY")
    print("=" * 20)
    print(f"✅ Enhanced metrics provide more nuanced quality assessment")
    print(f"✅ Efficiency scoring effectively balances compression vs quality") 
    print(f"✅ Clear recommendations emerge for different use cases")
    print(f"✅ Content type significantly impacts optimal pipeline choice")
