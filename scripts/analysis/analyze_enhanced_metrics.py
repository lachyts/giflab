...
def load_data():
    ...
    return df[df["success"] is True].copy()
...
def find_sweet_spot_pipelines(df):
    ...
    print("\nSweet Spot Results:")
    print(
        f"• Found {len(sweet_spot)} results meeting all criteria ({len(sweet_spot)/len(df)*100:.1f}%)"
    )
...
def detailed_case_studies(df):
    ...
    print(
        f"CASE 1: Biggest Quality Improvement (+{max_improvement['quality_improvement']:.3f})"
    )
    ...
    print(
        "• Why efficient: Excellent balance of high compression with good quality retention"
    )
    ...
    print("• Why balanced: Strong performance across all key metrics")
